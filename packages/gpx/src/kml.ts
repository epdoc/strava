...
export class KmlCommand extends BaseRootCmdClass<KmlCmdOptions> {
  override defineMetadata() {
    this.description = 'Generate KML files from Strava activities';
    this.name = 'kml';
  }
  ...
  helpText(): string {
    ...
    msg.h1('\nKML Generation\n');
    msg.text(
      'Generate KML files from Strava activities for use in GPS devices and mapping software.\n\n',
    );

    msg.h2('Output Behavior:\n');
    msg.label('  •').text('By default, all activities are combined into a single KML file\n');
    ...
    msg.text('      Generate KML with lap waypoints for rides only\n\n');
    msg.label('  ').value('--date 20240101-20241231 --laps --no-tracks\n');
    msg.text('      Generate KML with only lap waypoints (no tracks)\n');
