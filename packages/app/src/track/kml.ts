 ...
 * kml.setLineStyles( customStyles);
 * await kml.outputData( 'output.kml', activities, segments);
 * ```
 */
export class KmlWriter extends TrackWriter {
   ...
   * ```
   */
  setLineStyles(styles: Stream.KmlLineStyleDefs) {
    Object.entries(styles).forEach(([name, style]) => {
      if (isValidActivityType(name) && isValidLineStyle(style)) {
        this.lineStyles[name] = style;
      } else {
        this.log.warn.warn('Warning: ignoring line style error')
  ...
  async outputData(
    ...
    this.log.info.h2('Generating KML file').fs(fsFile).emit();
    this.log.indent();
    ...
    try {
      ...
      this.log.outdent();
      this.log.info.h2('Successfully wrote').fs(file).ewt(m0);
    } catch (err) {
      if (this.writer) {
        await this.writer.close();
      }
  ...
  async outputActivity(indent: number, activity: StravaApi.Activity): Promise<void> {
    ...
    const description = await this.#buildActivityDescription(activity);
  ...
  async #buildActivityDescription(activity: StravaApi.Activity): Promise<string | undefined> {
    ...
    const cachedSegments = await this.app.getCachedSegments();
