...
export class GpxWriter extends TrackWriter {
  ...
  async outputData(
    ...
    if (REG.isGpx.test(folderpath)) {
      ...
      await this.#outputActivitiesFile(fsFile, activities);
    } else {
      const fsFolder = new FS.Folder(folderpath);
      await fsFolder.ensureDir();
      const m0 = this.log.mark();
      this.log.info.h2('Generating GPX files in folder').fs(folderpath).emit();
      this.log.indent();
      for (const activity of activities) {
        await this.#outputActivityFile(fsFolder, activity);
      }
      this.log.outdent();
      this.log.info.h2('Generated').count(activities.length).h2('GPX file').h2('successfully').ewt(
        m0,
      );
    }
  }
    ...
    try {
      ...
      if (this.writeTracks()) {
        for (const activity of activities) {
          ...
          numTrackpoints += num;
          this.log.info.text('Adding').count(num).text('track point')
      ...
      this.log.outdent();
      this.log.info.text('Wrote').count(activities.length).text('track')
  ...
  async #outputActivityFile(
    ...
    try {
      await this.#header(metadata);
      const line = this.log.info.text('Wrote');
